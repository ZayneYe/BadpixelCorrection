...
class MaskedAutoencoderViT(nn.Module):
    ...
    def patchify(self, imgs):
        ...
        p0, p1 = self.patch_embed.patch_size
        assert imgs.shape[2] % p0 == 0 and imgs.shape[3] % p1 == 0
        in_chans = imgs.shape[1]
        # h = w = imgs.shape[2] // p
        h, w = imgs.shape[2] // p0, imgs.shape[3] // p1
        # x = imgs.reshape(shape=(imgs.shape[0], 3, h, p, w, p))
        x = imgs.reshape(shape=(imgs.shape[0], in_chans, h, p0, w, p1))
        x = torch.einsum('nchpwq->nhwpqc', x)
        # x = x.reshape(shape=(imgs.shape[0], h * w, p**2 * 3))
        x = x.reshape(shape=(imgs.shape[0], h * w, p0 * p1 * in_chans))
        return x
    ...
    def random_masking(self, x, mask_ratio):
        ...
        ids_remove = L//2
        x_masked = torch.cat((x[:, :ids_remove, :], x[:, ids_remove+1:, :]), dim=1)
        ids_restore = torch.cat([torch.arange(0,L//2),torch.arange(L//2+1,L),torch.tensor(L//2).unsqueeze(-1)]).repeat(N,1)
        ids_restore = ids_restore.to(x.device).to(torch.int64)
    ...
    def forward_loss(self, imgs, pred, mask, org_imgs, dataset):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        # pred = pred * 48.85 + 151.78
        # org_imgs = org_imgs * 48.85 + 151.78
        mean, std = self.get_mean_std(dataset)
        pred = pred * std + mean
        org_imgs = org_imgs * std + mean
        # return loss
        target = self.patchify(org_imgs)
        mask_ = self.patchify(mask)
        loss = (pred - target) ** 2
        # loss = loss.mean(dim=-1)  # [N, L], mean loss per patch

        loss = (loss * mask_).sum()  # total loss on bad pixels
        normalize_term = torch.sum(pow(org_imgs*mask, 2))
        loss = loss / normalize_term
        return loss
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        ...
        decoder_embed_dim=12, decoder_depth=2, decoder_num_heads=2,
...
if __name__ == "__main__":
    model = mae_vit_base_patch16(img_size=(15,15), in_chans=1)
